...
//    - Instruction: Ensure proper validation and error handling is included for appointment updates.
    ...
    public ResponseEntity<Map<String, String>> updateAppointment(Appointment appointment){
        ...
        if(result.isEmpty()){
            response.put("message", "No appointment found with id: " + appointment.getId());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
        }

        if(!Objects.equals(result.get().getPatient().getId(), appointment.getPatient().getId())){
            response.put("message", "Patient Id are not equal");
        ...
        response.put("message", "appointment unavailable");
...
//    - Instruction: Make sure that the method checks for the patient ID match before deleting the appointment.
    ...
    public ResponseEntity<Map<String, String>> cancelAppointment(Long id, String token){
        ...
        if(!Objects.equals(patient.getId(), appointment.get().getId())){
            response.put("message", "patient id are not equal");
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
        }
...
//    - Instruction: Ensure the correct use of transaction boundaries, especially when querying the database for appointments.
    ...
    public Map<String, Object> getAppointment(String patientName, LocalDate date, String token){
        ...
        if(patientName.equals("null")){
            appointmentList = appointmentRepository.findByDoctorIdAndAppointmentTimeBetween(doctorID, startOfDay, endOfDay);
        }
        else{
            appointmentList = appointmentRepository.findByDoctorIdAndPatient_NameContainingIgnoreCaseAndAppointmentTimeBetween(
        ...
        List<AppointmentDTO> appointmentDTOS = appointmentList.stream()
                .map(app -> new AppointmentDTO(
                        ...
                        app.getStatus())).toList();
        appointmentMap.put("appointments", appointmentDTOS);
